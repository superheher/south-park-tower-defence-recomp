...
| 19 | `perf-overlap-step1-swap-fence-eager-push…` | **2026-06-09 night, layer-B step 1.** (a) pacing-diag v2.1: split the CP frame slot — `wrm` (in-stream WAIT_REG_MEM block) and `swp` (IssueSwap) shares; showed at 60 fps the CP parks ~8.7 ms/frame on the guest fence and present costs only ~1 ms. (b) The fix: after `++counter_` in XE_SWAP, push the live counter to the guest fence immediately (`RefreshVblankFence`) + `NotifyVblank()` — the guest's polled fence value otherwise only changes on vsync ticks, so its whole frame loop snapped to the 60 Hz grid (clean 30/20 cadences at high draw counts). Value series unchanged, visibility latency <16.7 ms sooner. Gates: ab_both 40s×3 p10 +2.5 (≥ +1.0 bar), all reps pairwise ≥ base; m9 run: 60-capable ceiling ~1100→~1300 draws/swap, heaviest band (~1700+) 38–51 with no grid-locks, stage-complete screen 60.0; render screenshots clean. detdiff N/A (no translation-path change; timing-only). |
| 18 | `perf-host-freq_keeper-low-priority-spinner…` | **2026-06-09 night, layer-A fix.** Cvar `freq_keeper` (default off; gamectl passes `--freq_keeper=true`): one nice-19 spinner thread holds the package CPU frequency up while the title runs. Kills the cold-entry cadence trap on powersave/EPP=power hosts (m6a: 26–30 swaps @1.0–1.2 GHz from the first field seconds; m7 same cold-soak protocol with the keeper: 60.0 @2.9 GHz). Orthogonal to the render pipeline (no PM4/translation change ⇒ detdiff/ab not applicable; gated by the m6/m7 cold-soak A/B + render screenshots). Root-level host fix that replaces it: EPP=performance (see STATUS.md). |
