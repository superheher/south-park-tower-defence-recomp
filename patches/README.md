...
| 16 | `perf-trace-inline-TraceWriter-WritePacketStart-End…` | **This session.** `TraceWriter::WritePacketStart`/`WritePacketEnd` are called around EVERY PM4 packet by the CP hot loop; out-of-line, the bare call overhead is ~3.2% of CP on-cpu in a heavy combat dip even when not tracing (measured: `cp_oncpu_profile.sh`). Move both bodies into `trace_writer.h` as inline so the `file_` null-check folds to a load+branch with no call/ret. Byte-identical behaviour; `.so`-only; gate pass/equivalent; render-correct. Floor-neutral (~0.5ms/frame; the combat floor is the title's own fixed-60Hz cost, not the CP) — kept as a CPU efficiency win. See `../docs/FLOOR-STRUCTURAL-CONCLUSION-REPORT.md`. |
