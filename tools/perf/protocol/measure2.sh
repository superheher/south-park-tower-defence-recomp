#!/usr/bin/env bash
# measure2.sh -- Task-1 canonical run: video from TITLE (menu baseline) through
...
mark script_start
"$GC" play >"$SP/m2_play.out" 2>&1 &

ok=""
for _ in $(seq 1 450); do
  grep -q 'title up' "$SP/m2_play.out" 2>/dev/null && { ok=1; break; }
  grep -q 'boot failed' "$SP/m2_play.out" 2>/dev/null && break
  sleep 0.2
done
...
ffmpeg -hide_banner -loglevel warning -f x11grab -window_id "$ID" -framerate 60 \
  ...
  -y "$SP/m2_capture.mp4" >"$SP/m2_ffmpeg.out" 2>&1 &
FF_PID=$!
mark ffmpeg_start

ok=""
for _ in $(seq 1 500); do
  grep -q 'IN LEVEL' "$SP/m2_play.out" 2>/dev/null && { ok=1; break; }
  grep -qE 'boot failed|could not enter' "$SP/m2_play.out" 2>/dev/null && break
  sleep 0.2
done
[ -z "$ok" ] && { mark abort_no_level; kill "$FF_PID" 2>/dev/null; exit 1; }
mark in_level
PID=$(pgrep -x south_park_td | head -1)
import -window "$ID" "$SP/m2_level_t0.png" 2>/dev/null
...
mark perf1_start
perf record -F 499 -g -p "$PID" -o "$SP/m2_perf_first.data" -- sleep 15 >/dev/null 2>&1
mark perf1_end
import -window "$ID" "$SP/m2_level_t20.png" 2>/dev/null

sleep 25
mark perf2_start
perf record -F 499 -g -p "$PID" -o "$SP/m2_perf_later.data" -- sleep 15 >/dev/null 2>&1
mark perf2_end
import -window "$ID" "$SP/m2_level_t60.png" 2>/dev/null

wait "$FF_PID" 2>/dev/null
mark video_end
import -window "$ID" "$SP/m2_level_end.png" 2>/dev/null
cp "$LOG" "$SP/m2_run.log" 2>/dev/null
