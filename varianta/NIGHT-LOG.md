...
⇒ **Pillar A advanced, not finished:** first gate (pending-counter) cracked + the second gate localized. **NEXT for pillar A:** drive the completion handshake more faithfully — when the pump completes a kicked submission, ALSO run the producer/consumer (so the title's resource recycling + per-submission bookkeeping advance) and forward the segment-pointer fence (sub_821C5DF0) — then re-measure whether kicks climb past 14 and the content draw IB (≫266 dw / phys-window) gets kicked. Then pillar B (DRAW_INDX→Vulkan) renders it. Committed (gated, default-safe, NOT pushed).
