...
- ⇒ **The menu content is now fully characterized for translation:** alpha-composited 2D UI (quads + quad-list text) through 2 shaders to a tiled RGBA8 EDRAM surface with depth. Matches the loaded shader set (Simple/SPTextured/SpHud…) and the hand-ported SPIR-V in tools/shaderc/out (Simple/SimpleCol/SPTextured/SPUntextured.frag). **NEXT (increment 2):** decode the geometry — for each content DRAW_INDX (op 0x22), the index buffer (from the packet) + the vertex streams (fetch constants reg 0x4800, the vertex format) → actual vertices; then (increment 3) build the VkPipeline (2 shaders, alpha blend, the extracted state) + upload vertices/indices + vkCmdDraw into an offscreen RGBA8 RT → present. Diagnostic added (gated under REX_CHUNKDUMP): `[drawstate]` content-draw state dump. Default boot unregressed.
