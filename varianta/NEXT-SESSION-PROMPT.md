...
- **NEXT (2 пилона):** **(A) Flow the ring** — декрементить `device+0x2b04` как результат CP-completion киккнутой submission (faithful per-completion, НЕ cont.15 blanket force-to-0). Метрика = **kicks > 6 / WPTR > 37**. cont.15 доказал: форс counter'а ⇒ ring течёт (2725 kicks). **(B) DRAW_INDX→Vulkan** — VkPipeline из reg-file (0x4000 ALU, 0x4800 fetch→верт.стримы+текстуры, RT/viewport/blend) + .updb шейдер→SPIR-V → vkCmdDraw в swapchain.
