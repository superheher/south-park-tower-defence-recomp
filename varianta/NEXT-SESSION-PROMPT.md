...
- **⇒ /loop ИСЧЕРПАЛ ценность для этого билда (структурный лимит, «poor /loop fit» — подтверждено со всех сторон).** Остаток = ГЛУБОКИЙ multi-session GPU-resource-create сабсистем (Xenos tiled-texture decode + .xbv→SPIR-V + EDRAM RT + real draw-translation + per-item create-completion). Entry-points локализованы: sub_8211BE68 / sub_82247E70 / sub_822490D0(done-vs-requeue) / sub_8224F918. **⭐ЧТОБЫ ПРОДОЛЖИТЬ: re-issue /loop ИЛИ focused-сессия (свежий контекст) → начать GPU-create билд** (модель per-item create-completion, которую проверяет sub_822490D0; ИЛИ texture-decode первым). Gated диаги: REX_ADVGATE/REX_FORCEBE68/REX_ITEMSPIN/REX_LOADERLATCH/REX_CPDRAIN/REX_TEXWATCH/REX_TEXBIND.
